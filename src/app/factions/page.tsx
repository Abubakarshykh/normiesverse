import { fetchNormies, FACTION_COLORS } from '@/lib/api';
import Link from 'next/link';
import type { Metadata } from 'next';
import { Normie } from '@/lib/api';
...
export default async function FactionsPage() {
  ...
  const factionCounts: Record<string, { count: number; avgPower: number; members: Normie[] }> = {};

  normies.forEach((n) => {
    if (!factionCounts[n.faction]) factionCounts[n.faction] = { count: 0, avgPower: 0, members: [] };
    factionCounts[n.faction].count++;
    factionCounts[n.faction].avgPower += n.powerLevel;
    factionCounts[n.faction].members.push(n);
  });

  const factions = Object.entries(factionCounts).map(([name, data]) => ({
    name,
    count: data.count,
    avgPower: Math.round(data.avgPower / data.count),
    color: FACTION_COLORS[name] ?? '#888',
    lore: FACTION_LORE[name],
    preview: data.members.slice(0, 3),
  }));
    ...
    <div className="container mx-auto min-h-screen px-4 py-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {factions.map((faction) => (
          <Link key={faction.name} href={`/factions/${faction.name.toLowerCase()}`}>
            <div
              className="group relative overflow-hidden rounded-2xl border p-6 transition-all duration-300 hover:scale-[1.02] cursor-pointer"
              style={{
                borderColor: `${faction.color}30`,
                backgroundColor: `${faction.color}08`,
                boxShadow: `0 0 0 0 ${faction.color}`,
              }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 0 30px ${faction.color}30`; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 0 0 0 ${faction.color}`; }}
            >
              {/* Glow bg */}
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <h2 className="mt-2 text-2xl font-black uppercase tracking-wider text-white group-hover:text-white transition-colors" style={{ textShadow: `0 0 20px ${faction.color}60` }}>
                      {faction.name}
                    </h2>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground ml-1">View all →</span>
                </div>
              </div>
            </div>
          </Link>
